...
3. Review & Handoff: Upon completion, a plugin listening to `session.idle` triggers the Reviewer agent and posts a Discord notification for human-in-the-loop approval. Manual and scripted review execution should route through `/run-review`, which is intercepted in plugin `command.execute.before` and routed through a shared deterministic review executor so marker outputs are persisted for later phase-closeout gates.
...
When applied, orchestrator/communication plugins can set `output.noReply = true` for `/pipeline`, `/run-review`, and `/approve`, producing deterministic control responses without an LLM reasoning turn.
...
* Review artifacts: `_bmad-output/cycle-state/reviews/*.json` generated by `/run-review` with deterministic `round-<n>` versioning.
...
* Discord 2-Way Bot: The communication.ts plugin handles outbound events (session.idle/session.error/pipeline transitions) by posting summaries mapped to agent personas defined in demonlord.config.json. Inbound slash commands (`/approve`, `/party`, `/continue`, `/halt`, `/focus`, `/add-agent`, `/export`) are routed deterministically through authorization allowlists (user IDs, role IDs, optional channel ID) and denied fail-closed when unauthorized.
...
* Reliability/Validation Contract: Retry policy is fixed (`max_attempts=3`, backoff `0ms/250ms/1000ms`, no jitter), startup fails fast when required Discord env/config keys are missing, and verification is exposed through `npm run verify:beelzebub`.
