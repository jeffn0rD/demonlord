...
V1 review contract:
- `/creview` is a direct review command in the core loop.
- Base evidence comes from repository state (tasklist/plan/spec + git diff/commits + local verification output).
- Do not depend on shared review runner tools or persisted review artifact infrastructure.
