...
const SKILLS_ROOT = resolve(WORKTREE_ROOT, ".opencode", "skills");
...
const SKILL_ID_SCAN_FILES = [
  ...
  ".opencode/tests/harness/discord-contracts.v1.json",
];
...
function printHelp() {
  console.log("Usage: node .opencode/scripts/skill_docs_maintenance.mjs [--check|--write]");
...
function referenceExists(referencePath, allRelativePaths) {
  ...
  if (normalized.includes("*")) {
    ...
    return allRelativePaths.some((entry) => matcher.test(entry));
  }

  const normalizedDirectory = normalized.endsWith("/") ? normalized : `${normalized}/`;
  return allRelativePaths.includes(normalized) || allRelativePaths.includes(normalizedDirectory);
}
...
function renderIndex(skills) {
  const lines = [
    "# Agent Docs Index",
    "",
    "This file is generated by `.opencode/scripts/skill_docs_maintenance.mjs`.",
    "Do not edit manually. Run `npm run skills:maintain` from `.opencode`.",
    "",
    "## Purpose",
    "",
