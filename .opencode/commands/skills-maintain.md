...
2. In `write` mode, run:
   - `node .opencode/scripts/skill_docs_maintenance.mjs --write`
   - then `node .opencode/scripts/skill_docs_maintenance.mjs --check`
3. In `check` mode, run:
   - `node .opencode/scripts/skill_docs_maintenance.mjs --check`
