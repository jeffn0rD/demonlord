...
Hard constraints:
- Primary review evidence must come from persisted review artifacts.
...
1. Load persisted review artifacts from `_bmad-output/cycle-state/reviews/`.
2. For subphase evidence, include files matching:
   - `$1-phase-<phase>-subphase-*-round-*.json`
3. For phase-gate history, include files matching:
   - `$1-phase-<phase>-gate-round-*.json`
4. For module evidence, include:
   - `module-phase-<phase>-*-round-*.json`
5. If expected artifacts are missing, attempt fallback evidence by searching for marker terminators (`CYCLE_CREVIEW_RESULT`, `CYCLE_MREVIEW_RESULT`) across repo transcripts/artifacts.
6. If fallback evidence is still incomplete, continue review with available data and mark missing evidence explicitly.
